...
export const WORK = [
  {
    ...
    body: "Joined BuildOps as IC5 on a vertical SaaS platform for the trades industry, working across the stack in the financial-operations domain. The domain was new to me; the engineering wasn't. I had to learn how accounting periods and close semantics, enforcement rules that hold across object types and fiscal years, ledger integrity and the reporting layer on top actually work before I could be trusted to change any of it. I now spec and review changes in that area and coordinate its releases. That turned out to be the part agents can't do for you: the judgment about what should happen lives in understanding the domain, not in writing the code. Since mid-2025 I've operated almost entirely through AI-augmented workflows — orchestrating coding agents, custom skills, and slash commands instead of typing implementation by hand. Day-to-day spans agent design (Claude Code skills/commands/hooks for the team), AI-driven research and architecture (deep-research agents to evaluate trade-offs before writing a line), AI-paired testing and code review, AI-led accessibility audits aligned to WCAG 2.1 AA, and design-to-code translation via Figma/MCP integrations. The measurable result: sustained monthly merge throughput up roughly 7× over my own first-year baseline, while every change stayed human-reviewed before merge — 268 pull requests reviewed for other engineers over the same period. What it cost is worth naming: the hard part moved from writing code to reviewing structural decisions fast enough to keep pace, which forced explicit verification gates I never needed when I typed everything myself.",
    metrics: [
